...
class ContractExtractionDB(Base):
    ...
    parties = relationship("ContractPartyDB", back_populates="contract")
    fees = relationship("ContractFeeDB", back_populates="contract")
    pricing_rules = relationship("PricingRuleDB", back_populates="contract")
...
class ExtractionJobDB(Base):
    ...
    openai_file_id = Column(String(100))
    model_used = Column(String(50))
...
class ContractExtractor:
    ...
    def extract_from_file(self, 
        ...
        print(f"Analyzing contract: {filename}")
    ...
    def save_to_database(self, extraction: ContractExtraction, filename: str, file_content: bytes, db_id: int = None) -> int:
        ...
        try:
            ...
            try:
                if metadata.get('effective_date'):
                    effective_date = datetime.strptime(metadata['effective_date'], '%Y-%m-%d').date()
            ...
            db.commit()
            print(f"Saved to database with ID: {contract_db.id}")
...
async def health_check():
    return {"status": "healthy", "service": "contract-extraction-api"}
